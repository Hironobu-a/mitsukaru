# Nemielu - はじめに (Getting Started)

Nemieluプロジェクトへようこそ。このドキュメントは、プロジェクトに参加する最初のステップとして、環境構築からドキュメントの歩き方までを案内します。
...
```bash
alias ns="cd /path/to/nemielu-dev && ./docker-start.sh"
alias nt="cd /path/to/nemielu-dev && ./docker-test.sh"
```
...
**ブラウザで http://localhost:8000 を開いて、Nemieluを体験してください！**
